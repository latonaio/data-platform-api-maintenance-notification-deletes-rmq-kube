...
import (
	...
	"data-platform-api-maintenance-notification-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) headerDelete(
	...
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "MaintenanceNotificationHeader", "runtime_session_id": sessionID})
	if err != nil {
...
func (c *DPFMAPICaller) itemDelete(
	...
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": item, "function": "MaintenanceNotificationItem", "runtime_session_id": sessionID})
	if err != nil {
		err = xerrors.Errorf("rmq error: %w", err)
		log.Error("%+v", err)
		return nil
